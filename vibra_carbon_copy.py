...
class BalanceLoggerApp:
    ...
    def start_measurements(self):
        ...
        # Pre-wake and OPEN connections
        self.ser_objects = []  # Important: clear previous
        ...
        time.sleep(1)  # Give balance time to fully stabilize
    ...
    def add_balance(self):
        ...
        for port in ports:
            ...
            if device in description:
                all_port_choices.append(description)  # already contains device
            else:
                all_port_choices.append(f"{description} ({device})")

        # Now filter out ports already selected
        selected_ports = []
        for p in self.port_vars:
            selected = p.get()
            if "(" in selected and ")" in selected:
                selected_port = selected.split("(")[-1].replace(")", "")
        ...
        for choice in all_port_choices:
            if "(" in choice and ")" in choice:
                port_device = choice.split("(")[-1].replace(")", "")
                if port_device not in selected_ports:
                    available_port_choices.append(choice)
            else:
                available_port_choices.append(choice)  # fallback, rare

        if available_port_choices:
            longest_port = max(len(p) for p in available_port_choices)
            combobox_width = min(max(60, int(longest_port * 0.75)), 70)
        else:
            combobox_width = 60
    ...
    def refresh_balance_dropdowns(self):
        ...
        for port in ports:
            ...
            device = port.device
            if device in description:
                all_port_choices.append(description)
            else:
                all_port_choices.append(f"{description} ({device})")

        all_port_choices.sort()
        ...
        for i, port_var in enumerate(self.port_vars):
            ...
            available_choices = []
            for choice in all_port_choices:
                device_in_choice = choice.split("(")[-1].replace(")", "") if "(" in choice else choice

                if device_in_choice == self.get_device_from_selection(current_selected) or device_in_choice not in used_ports:
